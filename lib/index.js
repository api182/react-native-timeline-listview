...
export default class Timeline extends Component {
  constructor(props, context) {
    ...
    this.state = {
      data: this.convertTimelineArrayToDateMap(this.props.data),
      dataSource: ds.cloneWithRowsAndSections(this.convertTimelineArrayToDateMap(this.props.data)),
      x: 0,
      width: 0
    }
  }
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      data: this.convertTimelineArrayToDateMap(nextProps.data),
      dataSource: ds.cloneWithRowsAndSections(this.convertTimelineArrayToDateMap(nextProps.data))
    })
  }

  convertTimelineArrayToDateMap(arr) {
    var dateMap = {}; // Create the blank map
  ...
  _renderSectionHeader(sectionData, date) {
    ...
    return (
      <View style={styles.dateBadge}>
        <Text style={{ color: '#FFFFFF', fontWeight: "700" }}>{date}</Text>
      </View>
    )
  ...
  _renderEvent(rowData, sectionID, rowID) {
    ...
    let isLast = this.props.renderFullLine ? !this.props.renderFullLine : this.state.data[sectionID].slice(-1)[0] === rowData
    let lineColor = isLast ? ('rgba(0,0,0,0)') : (rowData.lineColor ? rowData.lineColor : this.props.lineColor)
    var opStyle = null

    let itemMarginTop = (!isLast ? 15 : 0);
    ...
    return (
      <View style={[styles.details, opStyle]} onLayout={(evt) => { if (!this.state.x && !this.state.width) { var { x, width } = evt.nativeEvent.layout; this.setState({ x, width }) } }}>
        ...
        >
          <View style={{ marginTop: itemMarginTop }}>
            {this.renderDetail(rowData, sectionID, rowID)}
          </View>
          {!isLast ? this._renderSeparator() : null}
        </TouchableOpacity>
      </View>
    )
  }
  ...
  _renderCircle(rowData, sectionID, rowID) {
    ...
    let isFirst = this.props.renderFullLine ? !this.props.renderFullLine : this.state.data[sectionID].slice(0)[0] === rowData;
